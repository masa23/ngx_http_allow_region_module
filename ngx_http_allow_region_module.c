...
static ngx_command_t ngx_http_allow_region_commands[] = {
    ...
    {ngx_string("allow_jp"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF |
                                 NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1,
     ngx_http_allow_region_rule, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},
    ...
    {ngx_string("region_check"),
     ...
     offsetof(ngx_http_allow_region_loc_conf_t, enable), NULL},
...
ngx_http_allow_region_inet6(ngx_http_request_t *r,
  ...
  if (alcf->rules6_cust) {
    ...
    for (i = 0; i < alcf->rules6_cust->nelts; i++) {
      for (n = 0; n < 16; n++) {
        if ((p[n] & rule6_cust[i].mask.s6_addr[n]) !=
            rule6_cust[i].addr.s6_addr[n]) {
          goto next2;
        }
      }
...
static char *ngx_http_allow_region_rule(ngx_conf_t *cf, ngx_command_t *cmd,
  ...
  if (cidr.family == AF_INET || all) {
    if (value[0].len == 15 &&
        ngx_strcmp(value[0].data, "custom_allow_jp") == 0) {
      if (alcf->rules_cust == NULL) {
        alcf->rules_cust =
            ngx_array_create(cf->pool, 4, sizeof(ngx_http_allow_region_rule_t));
        if (alcf->rules_cust == NULL) {
          return NGX_CONF_ERROR;
        }
      }
  ...
  if (cidr.family == AF_INET6 || all) {
    if (value[0].len == 15 &&
      ...
      if (alcf->rules6_cust == NULL) {
        alcf->rules6_cust = ngx_array_create(
            cf->pool, 4, sizeof(ngx_http_allow_region_rule6_t));
        if (alcf->rules6_cust == NULL) {
          return NGX_CONF_ERROR;
        }
      }
